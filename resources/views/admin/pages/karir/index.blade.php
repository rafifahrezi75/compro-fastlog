...
@section('content')
  <div x-data="karirManager()" x-init="init()" class="space-y-6 max-w-full overflow-x-hidden">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4">
      ...
      <div
        ...
        <div class="min-w-0">
          ...
          <h4 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mt-0.5" x-text="karirs.length">0</h4>
        </div>
      </div>
      ...
      <div
        ...
        <div class="min-w-0">
          ...
          <h4 class="text-lg sm:text-xl font-bold text-amber-600 dark:text-amber-400 mt-0.5" x-text="countTutup()">0</h4>
        </div>
      </div>
      ...
      <div
        ...
        <div class="min-w-0">
          ...
          <h4 class="text-lg sm:text-xl font-bold text-purple-600 dark:text-purple-400 mt-0.5" x-text="countCities()">0</h4>
        </div>
      </div>
    </div>
    ...
    <div
      ...
      <div
        ...
        <div class="flex items-center gap-2.5 flex-wrap justify-end">
          ...
          <!-- Reset Filter -->
          <button x-show="searchQuery || selectedStatus !== 'Semua' || selectedProvinsi !== 'Semua'" @click="resetFilters()"
              ...
              <tr class="hover:bg-gray-50/70 dark:hover:bg-gray-800/40 transition-colors">
                ...
                <td class="py-3.5 px-3.5 sm:px-5 min-w-[220px]">
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      <h4 class="font-semibold text-gray-900 dark:text-white" x-text="item.nama_karir"></h4>
                      <div class="flex items-center gap-2 mt-1 flex-wrap">
                        ...
                        <a :href="'/admin/pelamar?posisi=' + encodeURIComponent(item.nama_karir)"
                          ...
                          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
                          </svg>
                ...
                <td class="py-3.5 px-3.5 sm:px-5 min-w-[200px]">
                  <div class="flex items-start gap-1.5 text-gray-700 dark:text-gray-300">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-gray-400 line-clamp-1 mt-0.5" x-text="item.alamat_detail"></p>
                    </div>
                  </div>
                </td>
            ...
            <tr x-show="filteredKarirs.length === 0">
              <td colspan="6" class="py-12 text-center">
                <div class="flex flex-col items-center justify-center max-w-sm mx-auto">
                  ...
                  <h4 class="text-sm font-semibold text-gray-900 dark:text-white">Tidak Ada Data Karir</h4>
      ...
      <div
        ...
        <div>
          ...
          dari <span class="font-semibold text-gray-900 dark:text-white" x-text="filteredKarirs.length"></span> posisi karir
        </div>
      ...
      <div @click.outside="closeAddModal()" x-show="isAddModalOpen"
        ...
        <div class="px-6 py-4.5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400">Lengkapi detail posisi, wilayah penempatan, dan poin kualifikasi.</p>
            </div>
          </div>
          <button @click="closeAddModal()" class="p-1.5 text-gray-400 hover:text-gray-600 rounded-lg cursor-pointer">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
            </svg>
          </button>
        ...
        <form @submit.prevent="submitAddKarir()" class="p-6 space-y-4 max-h-[78vh] overflow-y-auto">
          ...
          <div>
            ...
            <input type="text" x-model="formAdd.nama_karir" @input="formAdd.slug = generateSlug(formAdd.nama_karir)"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" x-model="formAdd.departemen" placeholder="Contoh: Operations / Logistics / Finance"
                class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none" />
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select x-model="formAdd.kota" :disabled="!formAdd.provinsi_kode" required
                ...
                <option value="" disabled selected x-text="formAdd.provinsi_kode ? '-- Pilih Kota / Kabupaten --' : '-- Pilih Provinsi Terlebih Dahulu --'"></option>
          ...
          <div>
            ...
            <div class="space-y-2.5">
              <template x-for="(point, idx) in formAdd.kualifikasi_list" :key="idx">
                <div class="flex items-center gap-2">
                  <span class="w-6 h-6 rounded-lg bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 text-xs font-bold flex items-center justify-center shrink-0" x-text="idx + 1"></span>
                  <input type="text" x-model="formAdd.kualifikasi_list[idx]" placeholder="Contoh: Pendidikan minimal D3 / S1 semua jurusan"
                  ...
                  <button type="button" @click="removeKualifikasiPoint('add', idx)" :disabled="formAdd.kualifikasi_list.length === 1"
                    class="p-2 text-gray-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl transition disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer" title="Hapus Poin">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p class="text-[11px] text-gray-400 mt-1.5">Klik <strong>+ Tambah Poin</strong> untuk menambah butir persyaratan kualifikasi kerja.</p>
          </div>
      ...
      <div @click.outside="closeEditModal()" x-show="isEditModalOpen"
        ...
        <div class="px-6 py-4.5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400">Perbarui informasi posisi, lokasi, atau butir kualifikasi.</p>
            </div>
          </div>
          <button @click="closeEditModal()" class="p-1.5 text-gray-400 hover:text-gray-600 rounded-lg cursor-pointer">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
            </svg>
          </button>
        ...
        <form @submit.prevent="submitEditKarir()" class="p-6 space-y-4 max-h-[78vh] overflow-y-auto">
          ...
          <div>
            ...
            <input type="text" x-model="formEdit.nama_karir" @input="formEdit.slug = generateSlug(formEdit.nama_karir)"
              required
              class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none" />
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select x-model="formEdit.kota" :disabled="!formEdit.provinsi_kode" required
                class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none disabled:opacity-50 disabled:bg-gray-50 dark:disabled:bg-gray-900 disabled:cursor-not-allowed">
                <option value="" disabled selected x-text="formEdit.provinsi_kode ? '-- Pilih Kota / Kabupaten --' : '-- Pilih Provinsi Terlebih Dahulu --'"></option>
          ...
          <div>
            ...
            <div class="space-y-2.5">
              <template x-for="(point, idx) in formEdit.kualifikasi_list" :key="idx">
                <div class="flex items-center gap-2">
                  <span class="w-6 h-6 rounded-lg bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs font-bold flex items-center justify-center shrink-0" x-text="idx + 1"></span>
                  <input type="text" x-model="formEdit.kualifikasi_list[idx]" placeholder="Contoh: Pengalaman minimal 1-2 tahun di bidang terkait"
                    class="flex-1 px-3.5 py-2 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none" />
                  <button type="button" @click="removeKualifikasiPoint('edit', idx)" :disabled="formEdit.kualifikasi_list.length === 1"
                    class="p-2 text-gray-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl transition disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer" title="Hapus Poin">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                    </svg>
    ...
    <div x-show="isDetailModalOpen" x-cloak
      ...
      <div @click.outside="closeDetailModal()" x-show="isDetailModalOpen"
        x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95"
        x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150"
        x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
        class="w-full max-w-2xl rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-2xl overflow-hidden my-8">
        <template x-if="selectedItem">
          <div>
            ...
            <div class="px-6 py-4.5 border-b border-gray-100 dark:border-gray-800 flex items-start justify-between">
              <div>
                <div class="flex items-center gap-2 flex-wrap mb-1.5">
                  ...
                  <span
                    :class="selectedItem.status === 'Aktif' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'"
                    class="px-2.5 py-0.5 rounded-full text-xs font-semibold border"
                    x-text="selectedItem.status"></span>
                </div>
                <h3 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white" x-text="selectedItem.nama_karir"></h3>
              </div>
              <button @click="closeDetailModal()" class="p-1.5 text-gray-400 hover:text-gray-600 rounded-lg cursor-pointer">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
              </button>
            ...
            <div class="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
              <!-- Info Pelamar Masuk -->
              <div class="p-4 rounded-xl bg-brand-50/60 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-xl bg-brand-500 text-white flex items-center justify-center font-bold text-sm shadow-sm">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    <span class="text-xs text-gray-500 dark:text-gray-400 font-medium">Total Kandidat Pelamar:</span>
                    <h4 class="text-base font-bold text-gray-900 dark:text-white" x-text="(selectedItem.pelamars_count || 0) + ' Orang Mendaftar'"></h4>
                  </div>
                </div>
              ...
              <div class="p-4 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/60 space-y-2">
                <div class="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-brand-600 dark:text-brand-400">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
                  <div>
                    ...
                    <p class="font-semibold text-gray-900 dark:text-white mt-0.5" x-text="selectedItem.kota"></p>
                  </div>
                  <div>
                    <span class="text-gray-400">Provinsi:</span>
                    <p class="font-semibold text-gray-900 dark:text-white mt-0.5" x-text="selectedItem.provinsi"></p>
                  </div>
                  <div>
                    <span class="text-gray-400">Negara:</span>
                    <p class="font-semibold text-gray-900 dark:text-white mt-0.5" x-text="selectedItem.negara || 'Indonesia'"></p>
                  </div>
                </div>
                <div class="pt-2 border-t border-gray-200 dark:border-gray-700 text-xs">
                  <span class="text-gray-400">Alamat Lengkap:</span>
                  <p class="font-medium text-gray-800 dark:text-gray-200 mt-1" x-text="selectedItem.alamat_detail"></p>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="p-4 bg-gray-50 dark:bg-gray-800/40 rounded-xl border border-gray-100 dark:border-gray-800">
                  <template x-if="selectedItem.kualifikasi_array && selectedItem.kualifikasi_array.length > 0">
                    <ul class="space-y-2">
                      <template x-for="(req, rIdx) in selectedItem.kualifikasi_array" :key="rIdx">
                  ...
                  <template x-if="!selectedItem.kualifikasi_array || selectedItem.kualifikasi_array.length === 0">
                    <p class="text-xs text-gray-400" x-text="selectedItem.kualifikasi || 'Tidak ada persyaratan khusus.'"></p>
                  </template>
                </div>
              </div>
            </div>
            ...
            <!-- Footer Detail -->
            <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-white/[0.02] flex items-center justify-end gap-2.5">
      ...
      return {
        ...
        get filteredKarirs() {
          return this.karirs.filter(item => {
            ...
            const matchesStatus = this.selectedStatus === 'Semua' || item.status === this.selectedStatus;
            const matchesProv = this.selectedProvinsi === 'Semua' || item.provinsi === this.selectedProvinsi;
        ...
        openEditModal(item) {
          ...
          let matchedProv = this.masterProvinsiList.find(p => p.nama.toLowerCase() === (item.provinsi || '').toLowerCase());
          let provKode = matchedProv ? matchedProv.kode : '';
          let kotaList = provKode ? (this.masterKotaMap[provKode] || []) : [];

          // Parse qualification points
          let kualifikasiList = [''];
          if (item.kualifikasi_array && Array.isArray(item.kualifikasi_array) && item.kualifikasi_array.length > 0) {
