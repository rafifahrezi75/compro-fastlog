...
@section('content')
    <div x-data="infoManager()" x-init="init()" class="space-y-6 max-w-full overflow-x-hidden">
        ...
        @if (session('error'))
            <div class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 border border-red-200"
                role="alert">
                {{ session('error') }}
            </div>
        @endif
                            ...
                            <tr class="hover:bg-gray-50/70 dark:hover:bg-gray-800/40 transition-colors">
                                ...
                                <td class="py-3.5 px-3.5 sm:px-5 min-w-[220px]">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center shrink-0 border border-gray-200 overflow-hidden">
                                            <template x-if="item.logo">
                                                <img :src="item.logo.startsWith('images/') ? '/' + item.logo : '/storage/' + item
                                                    .logo"
                                ...
                                <td class="py-3.5 px-3.5 sm:px-5">
                                    <p class="font-medium text-gray-900 dark:text-white"><span x-text="item.kota"></span>
                                    </p>
                        ...
                        <tr x-show="filteredInfos.length === 0">
                            <td colspan="5" class="py-12 text-center">
                                <div class="flex flex-col items-center justify-center max-w-sm mx-auto text-gray-400">
                                    ...
                                    <h4 class="text-sm font-semibold text-gray-900 dark:text-white">Tidak Ada Data Info
                                    </h4>
                                    <p class="text-xs text-gray-500 mt-1">Data info perusahaan kosong atau tidak ada hasil
                                        pencarian.</p>
                                </div>
                            </td>
                        </tr>
            ...
            <div
                ...
                <div>
                    Menampilkan <span class="font-semibold text-gray-900 dark:text-white"
                        x-text="filteredInfos.length > 0 ? ((currentPage - 1) * perPage) + 1 : 0"></span> -
                    <span class="font-semibold text-gray-900 dark:text-white"
                        x-text="Math.min(currentPage * perPage, filteredInfos.length)"></span> dari
                ...
                <template x-if="filteredInfos.length > perPage">
                    <div class="flex items-center gap-1.5">
                        <button @click="prevPage()" :disabled="currentPage === 1"
                            class="px-3 py-1.5 rounded-lg border border-gray-200 disabled:opacity-40 cursor-pointer">Sebelumnya</button>
                        <template x-for="page in totalPages" :key="page">
                            <button @click="goToPage(page)"
                                :class="currentPage === page ? 'bg-brand-500 text-white' : 'border border-gray-200'"
                                class="w-8 h-8 rounded-lg flex items-center justify-center cursor-pointer"
                                x-text="page"></button>
                        </template>
                        <button @click="nextPage()" :disabled="currentPage === totalPages"
                            class="px-3 py-1.5 rounded-lg border border-gray-200 disabled:opacity-40 cursor-pointer">Selanjutnya</button>
                    </div>
                </template>
            </div>
        </div>
        ...
        <div x-show="isDeleteModalOpen" x-cloak
            ...
            <div
                class="w-full max-w-md rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 shadow-2xl overflow-hidden p-6 text-center">
                <div
                    class="w-16 h-16 rounded-full bg-rose-100 text-rose-500 flex items-center justify-center mx-auto mb-4">
                ...
                <p class="text-sm text-gray-500 mb-6">Tindakan ini tidak dapat dibatalkan. Info "<span
                        x-text="formDelete.nama" class="font-semibold text-gray-800"></span>" akan dihapus secara
                    permanen.</p>
                <form :action="'/admin/infos/' + formDelete.id" method="POST"
                    ...
                    <button type="button" @click="closeDeleteModal()"
                        class="px-5 py-2.5 text-sm font-medium text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 cursor-pointer">Batal</button>
                    <button type="submit"
                        class="px-5 py-2.5 text-sm font-medium text-white bg-rose-500 rounded-xl hover:bg-rose-600 cursor-pointer">Ya,
                        Hapus!</button>
                </form>
            </div>
        </div>
