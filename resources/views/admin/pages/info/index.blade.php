...
@section('page_title', 'Master Info')
@section('content')
    <div x-data="infoManager()" x-init="init()" class="space-y-6 max-w-full overflow-x-hidden">
                    ...
                    <span class="p-2 rounded-xl bg-brand-500/10 text-brand-500 dark:bg-brand-500/20">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                                d="M11 11h2v6h-2v-6zM11 7h2v2h-2V7zm1 15C6.47715 22 2 17.5228 2 12S6.47715 2 12 2s10 4.47715 10 10-4.47715 10-10 10z">
                            </path>
                        </svg>
                    </span>
                    Master Data Info Perusahaan
                </h1>
                <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Kelola informasi perusahaan seperti nama, kontak, alamat, dan logo.
                </p>
            </div>
        ...
        <div
            ...
            <div
                ...
                <div class="relative flex-1 min-w-[240px] max-w-lg">
                    ...
                    <input type="text" x-model="searchQuery" @input="currentPage = 1"
                        placeholder="Cari info perusahaan, kota..."
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-white/[0.01]">
                            ...
                            <th class="py-3.5 px-3.5 sm:px-5 text-[11px] font-semibold tracking-wider text-gray-500 dark:text-gray-400 uppercase">Perusahaan</th>
                            <th class="py-3.5 px-3.5 sm:px-5 text-[11px] font-semibold tracking-wider text-gray-500 dark:text-gray-400 uppercase">Kontak</th>
                            <th class="py-3.5 px-3.5 sm:px-5 text-[11px] font-semibold tracking-wider text-gray-500 dark:text-gray-400 uppercase">Lokasi</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800 text-xs sm:text-sm">
                        <template x-for="(item, index) in paginatedInfos" :key="item.id">
                            <tr class="hover:bg-gray-50/70 dark:hover:bg-gray-800/40 transition-colors">
                                ...
                                <td class="py-3.5 px-3.5 sm:px-5 min-w-[220px]">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center shrink-0 border border-gray-200 overflow-hidden">
                                            <template x-if="item.logo">
                                                <img :src="item.logo.startsWith('images/') ? '/' + item.logo : '/storage/' + item.logo" class="w-full h-full object-cover" />
                                            </template>
                                            <template x-if="!item.logo">
                                        ...
                                        <div>
                                            <h4 class="font-semibold text-gray-900 dark:text-white" x-text="item.nama"></h4>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-3.5 px-3.5 sm:px-5 text-gray-700 dark:text-gray-300">
                                    ...
                                    <div class="flex items-center gap-1.5 text-xs">
                                        ...
                                        <span x-text="item.notelp"></span>
                                    </div>
                                </td>
                                <td class="py-3.5 px-3.5 sm:px-5">
                                    <p class="font-medium text-gray-900 dark:text-white"><span x-text="item.kota"></span></p>
                                    <p class="text-xs text-gray-400 line-clamp-1 mt-0.5" x-text="item.alamatLengkap"></p>
                                </td>
                        ...
                        <tr x-show="filteredInfos.length === 0">
                            <td colspan="5" class="py-12 text-center">
                                <div class="flex flex-col items-center justify-center max-w-sm mx-auto text-gray-400">
                                    <svg class="w-7 h-7 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 11h2v6h-2v-6zM11 7h2v2h-2V7zm1 15C6.47715 22 2 17.5228 2 12S6.47715 2 12 2s10 4.47715 10 10-4.47715 10-10 10z"></path></svg>
                                    <h4 class="text-sm font-semibold text-gray-900 dark:text-white">Tidak Ada Data Info</h4>
                                    <p class="text-xs text-gray-500 mt-1">Data info perusahaan kosong atau tidak ada hasil pencarian.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="p-3.5 sm:p-4 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs text-gray-500 dark:text-gray-400 bg-gray-50/30 dark:bg-white/[0.01]">
                <div>
                    Menampilkan <span class="font-semibold text-gray-900 dark:text-white" x-text="filteredInfos.length > 0 ? ((currentPage - 1) * perPage) + 1 : 0"></span> - 
                    <span class="font-semibold text-gray-900 dark:text-white" x-text="Math.min(currentPage * perPage, filteredInfos.length)"></span> dari 
                    <span class="font-semibold text-gray-900 dark:text-white" x-text="filteredInfos.length"></span> Info
                </div>
                <template x-if="filteredInfos.length > perPage">
        ...
        <div x-show="isAddModalOpen" x-cloak class="fixed inset-0 z-99999 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm overflow-y-auto" @keydown.escape.window="closeAddModal()">
            <div @click.outside="closeAddModal()" class="w-full max-w-2xl rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-2xl overflow-hidden my-8">
                ...
                <form action="{{ route('admin.infos.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4 max-h-[78vh] overflow-y-auto">
                    @csrf
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Nama Perusahaan *</label>
                        <input type="text" name="nama" required class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="notelp" required class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Kota *</label>
                            <input type="text" name="kota" required class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Logo</label>
                            <input type="file" name="logo" accept="image/*" class="w-full px-3.5 py-2 text-sm border border-gray-200 rounded-xl" />
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Alamat Lengkap *</label>
                        <textarea name="alamatLengkap" required rows="2" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500"></textarea>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Link Facebook</label>
                            <input type="url" name="linkFacebook" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Link Instagram</label>
                            <input type="url" name="linkInstagram" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Link X (Twitter)</label>
                            <input type="url" name="linkX" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Link LinkedIn</label>
                            <input type="url" name="linkLinkedin" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                    </div>
                    <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-brand-500 rounded-xl hover:bg-brand-600 cursor-pointer">Simpan Info</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="isEditModalOpen" x-cloak class="fixed inset-0 z-99999 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm overflow-y-auto" @keydown.escape.window="closeEditModal()">
            <div @click.outside="closeEditModal()" class="w-full max-w-2xl rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-2xl overflow-hidden my-8">
                <div class="px-6 py-4.5 border-b border-gray-100 flex items-center justify-between">
                ...
                <form :action="'/admin/infos/' + formEdit.id" method="POST" enctype="multipart/form-data" class="p-6 space-y-4 max-h-[78vh] overflow-y-auto">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Nama Perusahaan *</label>
                        <input type="text" name="nama" x-model="formEdit.nama" required class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Email *</label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">No Telp *</label>
                            <input type="text" name="notelp" x-model="formEdit.notelp" required class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Kota *</label>
                            <input type="text" name="kota" x-model="formEdit.kota" required class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Logo Baru (opsional)</label>
                            <input type="file" name="logo" accept="image/*" class="w-full px-3.5 py-2 text-sm border border-gray-200 rounded-xl" />
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Alamat Lengkap *</label>
                        <textarea name="alamatLengkap" x-model="formEdit.alamatLengkap" required rows="2" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500"></textarea>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Link Facebook</label>
                            <input type="url" name="linkFacebook" x-model="formEdit.linkFacebook" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Link Instagram</label>
                            <input type="url" name="linkInstagram" x-model="formEdit.linkInstagram" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Link X (Twitter)</label>
                            <input type="url" name="linkX" x-model="formEdit.linkX" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 uppercase mb-1.5">Link LinkedIn</label>
                            <input type="url" name="linkLinkedin" x-model="formEdit.linkLinkedin" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-brand-500" />
                        </div>
                    </div>
                    <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
        ...
        <div x-show="isDeleteModalOpen" x-cloak class="fixed inset-0 z-99999 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm" @keydown.escape.window="closeDeleteModal()">
            <div @click.outside="closeDeleteModal()" class="w-full max-w-md rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 shadow-2xl overflow-hidden p-6 text-center">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-2">Hapus Info Perusahaan?</h3>
                <p class="text-sm text-gray-500 mb-6">Tindakan ini tidak dapat dibatalkan. Info "<span x-text="formDelete.nama" class="font-semibold text-gray-800"></span>" akan dihapus secara permanen.</p>
                <form :action="'/admin/infos/' + formDelete.id" method="POST" class="flex items-center justify-center gap-3">
                    @csrf
                    @method('DELETE')
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('infoManager', () => ({
            ...
            get totalPages() {
                return Math.ceil(this.filteredInfos.length / this.perPage);
            },
