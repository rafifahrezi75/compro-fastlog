  ...
  <div x-data="testimoniManager()" x-init="init()" class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4">
      ...
      <div class="p-4 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-3.5">
        ...
        <div class="min-w-0">
          ...
          <h4 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mt-0.5" x-text="testimonis.length">0</h4>
        </div>
      </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-left border-collapse table-auto min-w-[600px]">
          <thead>
            <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/40 dark:bg-white/[0.02]">
              <th class="py-3 px-3.5 sm:px-5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[30%]">Nama & Perusahaan</th>
              <th class="py-3 px-3.5 sm:px-5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[40%]">Ulasan</th>
              <th class="py-3 px-3.5 sm:px-5 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[15%]">Status</th>
              ...
              <tr class="hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors group">
                <td class="py-3.5 px-3.5 sm:px-5">
                  <div class="flex items-start gap-3 min-w-0">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-brand-50 dark:bg-brand-950/40 text-brand-600 dark:text-brand-400 flex items-center justify-center shrink-0 border border-brand-100 dark:border-brand-800 font-bold text-sm">
                    ...
                    <div class="min-w-0 flex-1 pt-0.5">
                      ...
                      <p class="text-[11px] sm:text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-1" x-text="item.perusahaan || '-'"></p>
                    </div>
                  </div>
                </td>
                <td class="py-3.5 px-3.5 sm:px-5">
                  <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 line-clamp-2 italic" x-text="`&quot;${item.testimoni}&quot;`"></p>
                </td>
                <td class="py-3.5 px-3.5 sm:px-5">
                  <span x-show="item.status === 'published'" class="inline-flex items-center px-2 py-1 rounded-md text-[10px] sm:text-xs font-medium bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-200/50 dark:border-emerald-500/20">Published</span>
                  <span x-show="item.status === 'draft'" class="inline-flex items-center px-2 py-1 rounded-md text-[10px] sm:text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400 border border-gray-200/50 dark:border-gray-700">Draft</span>
                </td>
            ...
            <tr x-show="filteredTestimonis.length === 0">
              <td colspan="3" class="py-12 text-center">
                <div class="flex flex-col items-center justify-center max-w-sm mx-auto">
                  ...
                  <h4 class="text-sm font-semibold text-gray-900 dark:text-white">Tidak Ada Data Testimoni</h4>
      ...
      <div class="p-3.5 sm:p-4 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs text-gray-500 dark:text-gray-400 bg-gray-50/30 dark:bg-white/[0.01]">
        <div>
          Menampilkan
          <span class="font-semibold text-gray-900 dark:text-white" x-text="filteredTestimonis.length > 0 ? ((currentPage - 1) * perPage) + 1 : 0"></span> -
          <span class="font-semibold text-gray-900 dark:text-white" x-text="Math.min(currentPage * perPage, filteredTestimonis.length)"></span>
          dari <span class="font-semibold text-gray-900 dark:text-white" x-text="filteredTestimonis.length"></span> testimoni
        </div>
      ...
      <div @click.outside="closeAddModal()" x-show="isAddModalOpen"
        ...
        <form @submit.prevent="submitAdd()" class="p-6 space-y-4">
          <div>
            ...
            <input type="text" x-model="formAdd.nama" placeholder="Cth: Budi Santoso" required class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none" />
          </div>
          <div>
            ...
            <input type="text" x-model="formAdd.perusahaan" placeholder="Cth: PT Maju Jaya" class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none" />
          </div>
          <div>
            ...
            <textarea x-model="formAdd.testimoni" placeholder="Tuliskan ulasan pelanggan di sini..." required rows="4" class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none"></textarea>
          </div>
          <div>
    ...
    <div x-show="isEditModalOpen" x-cloak
      ...
      <div @click.outside="closeEditModal()" x-show="isEditModalOpen"
        ...
        <form @submit.prevent="submitEdit()" class="p-6 space-y-4">
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Perusahaan</label>
            <input type="text" x-model="formEdit.perusahaan" class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none" />
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Isi Testimoni <span class="text-rose-500">*</span></label>
            <textarea x-model="formEdit.testimoni" required rows="4" class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none"></textarea>
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Status <span class="text-rose-500">*</span></label>
    ...
    <div x-show="isDeleteModalOpen" x-cloak
      ...
      <div @click.outside="closeDeleteModal()" x-show="isDeleteModalOpen"
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mb-6">Apakah Anda yakin ingin menghapus testimoni dari <span class="font-semibold text-gray-800 dark:text-gray-200" x-text="itemToDelete?.nama"></span>? Data tidak dapat dikembalikan.</p>
...
<script>
  document.addEventListener('alpine:init', () => {
    Alpine.data('testimoniManager', () => ({
      ...
      itemToDelete: null,
      formAdd: { nama: '', perusahaan: '', testimoni: '', status: 'published' },
      formEdit: { id: null, nama: '', perusahaan: '', testimoni: '', status: 'published' },
      currentPage: 1,
      perPage: 10,
      ...
      openAddModal() {
        this.formAdd = { nama: '', perusahaan: '', testimoni: '', status: 'published' };
      ...
      async submitEdit() {
        this.isSubmitting = true;
        try {
          const res = await fetch(`{{ url('admin/testimoni') }}/${this.formEdit.id}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
              'Accept': 'application/json',
      ...
      async confirmDelete() {
        ...
        this.isSubmitting = true;
        try {
          const res = await fetch(`{{ url('admin/testimoni') }}/${this.itemToDelete.id}`, {
            method: 'DELETE',
            headers: {
              'Accept': 'application/json',
              'X-Requested-With': 'XMLHttpRequest',
          ...
          const result = await res.json();
          if (result.status === 'success') {
            this.testimonis = this.testimonis.filter(t => t.id !== this.itemToDelete.id);
