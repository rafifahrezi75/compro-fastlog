    ...
    <div x-data="testimoniManager()" x-init="init()" class="space-y-6">
        ...
        <div x-data="{
                ...
                init() {
                    ...
                    if (p.get('saved') === '1') { this.msg = 'Testimoni baru berhasil disimpan.'; this.show = true; }
                    else if (p.get('updated') === '1') { this.msg = 'Perubahan testimoni berhasil disimpan.'; this.show = true; }
                    if (!this.show) return;
                    setTimeout(() => {
                        this.show = false;
                        window.history.replaceState({}, '', '{{ route('admin.testimoni.index') }}');
                    }, 5000);
                }
            }"
            x-show="show" x-cloak
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4">
            ...
            <div
                ...
                <div class="min-w-0">
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 truncate">Published</p>
                    <h4 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mt-0.5"
                        x-text="testimonis.filter(t => t.status === 'published').length">0</h4>
                </div>
            </div>
            ...
            <div
                class="p-4 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-3.5">
                <div
                    class="w-11 h-11 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="min-w-0">
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 truncate">Draft</p>
                    <h4 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mt-0.5"
                        x-text="testimonis.filter(t => t.status === 'draft').length">0</h4>
                </div>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="py-3.5 px-3.5 sm:px-5 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a :href="'{{ url('admin/testimoni') }}/' + item.id" title="Lihat Detail"
                                        ...
                                        <a :href="'{{ url('admin/testimoni') }}/' + item.id + '/edit'" title="Ubah Testimoni"
