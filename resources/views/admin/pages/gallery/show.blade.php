...
@section('page_title', 'Detail Gallery')
@section('content')
    <div class="space-y-5">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">Detail Gallery</h1>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Pratinjau konten galeri sebagaimana tersimpan</p>
            </div>
        ...
        <div class="w-full rounded-2xl border border-gray-200 bg-white shadow-xs overflow-hidden dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="relative h-56 sm:h-72 bg-gray-200 dark:bg-gray-800">
                @if ($gallery->gambar)
                    <img src="{{ asset($gallery->gambar) }}" alt="{{ $gallery->judul }}" class="w-full h-full object-cover" />
                @else
                    <img src="https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=1200&q=80" alt="{{ $gallery->judul }}" class="w-full h-full object-cover" />
                @endif
                <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/25 to-transparent"></div>
                <span class="absolute top-4 right-4 inline-flex items-center px-3 py-1 rounded-md text-[10px] sm:text-xs font-medium {{ $gallery->status === 'published' ? 'bg-emerald-500/90 text-white' : ($gallery->status === 'draft' ? 'bg-gray-700/90 text-white' : 'bg-amber-600/90 text-white') }}">
                    {{ ucfirst($gallery->status) }}
                </span>
                <div class="absolute bottom-5 left-6 right-6">
                    <h2 class="text-lg sm:text-2xl font-bold text-white">{{ $gallery->judul }}</h2>
                    <p class="text-xs text-white/80 font-mono mt-0.5">/gallery — slug: {{ $gallery->slug }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div class="lg:col-span-2">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 shadow-xs dark:border-gray-800 dark:bg-white/[0.03]">
                    <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-3">Deskripsi</h3>
                    <div class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed space-y-3">{!! $gallery->deskripsi !!}</div>
                </div>
            </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-5 sm:p-6 shadow-xs dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <dl class="space-y-2.5 text-sm">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <dd class="font-medium text-gray-800 dark:text-gray-200 text-right">{{ $gallery->created_at?->translatedFormat('d M Y, H:i') ?? '-' }}</dd>
                        </div>
                        <div class="flex items-start justify-between gap-3">
                            <dt class="text-xs text-gray-500 dark:text-gray-400 shrink-0 pt-0.5">Terakhir Diubah</dt>
                            <dd class="font-medium text-gray-800 dark:text-gray-200 text-right">{{ $gallery->updated_at?->translatedFormat('d M Y, H:i') ?? '-' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
